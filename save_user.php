...
// Insertar persona
$stmt_persona = $conn->prepare("INSERT INTO persona (nom1, nom2, apell1, apell2, direccion, tele, movil, correo, fecha_nac, estado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
$stmt_persona->bind_param("ssssssssss", $nom1, $nom2, $apell1, $apell2, $direccion, $tele, $movil, $correo, $fecha_nac, $estado);

if ($stmt_persona->execute()) {
    $idpersona = $stmt_persona->insert_id;

    // Insertar usuario
    $stmt_usuario = $conn->prepare("INSERT INTO usuario (nombreu, contrasena, idperfil, idpersona, estado) VALUES (?, ?, ?, ?, ?)");
    $stmt_usuario->bind_param("ssiis", $nombreu, $contrasena, $idperfil, $idpersona, $estado);
